...
$(PREFIX)/bin/metview:
	$(MAKE) --file=$(MFEXT_HOME)/share/Makefile.standard PREFIX=$(PREFIX) EXPLICIT_NAME="$(EXPLICIT_NAME)" EXTRALDFLAGS="-L$(PREFIX)/../core/lib -ltirpc" OPTIONS="-DCMAKE_INSTALL_PREFIX=$(PREFIX) -DCMAKE_PREFIX_PATH='$(PREFIX);$(PREFIX)/../scientific_core;$(PREFIX)/../core' -DENABLE_UI=OFF -DENABLE_FORTRAN=ON -DENABLE_METVIEW_FORTRAN=ON -DNETCDF_PATH=$(PREFIX)/../scientific_core -DRPC_INCLUDE_DIR=$(PREFIX)/../core/include/tirpc -DRPC_LIBRARY=$(PREFIX)/../core/lib" download uncompress configure_cmake3 build_cmake install_cmake
